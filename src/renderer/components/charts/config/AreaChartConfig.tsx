...
const AreaChartConfig: React.FC<ChartTypeConfigProps> = ({
  config,
  onConfigChange,
  columns,
  numericColumns,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {/* Series Field (Optional for Multi-area) */}
      <div>
        <label style={{
          display: 'block',
          fontSize: '14px',
          ...
          Series Field (Optional - for multiple areas)
        </label>
        <select
        ...
        >
          <option value="">Single area chart</option>
          {columns.map(col => (
            <option key={col.name} value={col.name}>{col.name}</option>
          ))}
        </select>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <input
              type="checkbox"
              checked={config.areaSmooth || false}
              onChange={(e) => onConfigChange({ areaSmooth: e.target.checked })}
            />
            Smooth curves
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <input
              type="checkbox"
              checked={config.stackedArea || false}
              onChange={(e) => onConfigChange({ stackedArea: e.target.checked })}
            />
            Stacked areas
          </label>
      ...
      <div>
        ...
        <select
          value={config.areaOpacity || '0.7'}
          onChange={(e) => onConfigChange({ areaOpacity: parseFloat(e.target.value) })}
          style={{
            width: '100%',
            padding: '8px 12px',
            borderRadius: '6px',
        ...
        >
          ...
          <option value="0.9">Bold (90%)</option>
        </select>
      </div>
    </div>
  );
